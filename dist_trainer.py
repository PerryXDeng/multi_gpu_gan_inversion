...
class Trainer(nn.Module):
    def __init__(self, config, opts):
        ...
        # TODO: check if submodules registered correctly for DDP
        ...
        # Optimizers
        # Latent encoder
        self.enc_params = list(self.enc.parameters())  # TODO: check if submodules registered correctly for DDP
        ...
        ##########################
        # parts originally from trainer.initialize()
        stylegan_model_path = opts.stylegan_model_path
        # load StyleGAN model
        ...
        # self.StyleGAN.to(self.device) # TODO: check if this is necessary in case of DDP
        # get StyleGAN average latent in w space and the noise inputs
        # self.dlatent_avg = stylegan_state_dict['latent_avg'].to(self.device) # TODO: check if to(device) is necessary in case of DDP
        self.dlatent_avg = Parameter(stylegan_state_dict['latent_avg'], requires_grad=False) # TODO: check if submodules registered correctly for DDP
        # self.noise_inputs = [getattr(self.StyleGAN.noises, f'noise_{i}').to(self.device) for i in range(self.StyleGAN.num_layers)] # TODO: check if to(device) is necessary in case of DDP
        self.noise_inputs = [Parameter(getattr(self.StyleGAN.noises, f'noise_{i}'), requires_grad=False) for i in
                             range(self.StyleGAN.num_layers)] # TODO: check if submodules registered correctly for DDP
        # load Arcface weight
        self.Arcface.load_state_dict(torch.load(self.opts.arcface_model_path))
        for param in self.Arcface.parameters():
            param.requires_grad = False
        self.Arcface.eval() # TODO: check if this manual management causes bug in case of DDP
        ...
        for param in self.parsing_net.parameters():
            param.requires_grad = False
        self.parsing_net.eval() # TODO: check if this manual management causes bug in case of DDP
    ...
    def get_image(self, w=None, img=None, noise=None, zero_noise_input=True, training_mode=True):
        ...
        if 'use_fs_encoder' in self.config and self.config['use_fs_encoder']:
            ...
            # https://github.com/pytorch/pytorch/pull/48891 https://discuss.pytorch.org/t/c-cuda-extension-with-multiple-gpus/91241/6
            # cuda extension breaks on parallel on older pytorch versions
    ...
    def save_checkpoint(self, n_epoch, log_dir, enc_opt, enc_scheduler):
        checkpoint_state = {
            'n_epoch': n_epoch,
